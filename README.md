...
- El directorio **src** ahora cuenta con un nuevo directorio llamado **notificaciones**, el cual representa un servicio de mensajería que recibe eventos de integración propagados del sistema de PropiedadesAlpes, por medio de un broker de eventos.
...
- Nuestro proyecto de PropiedadesAlpes ha cambiado de forma considerable. Los siguientes son los cambios relevantes en cada módulo:
    - **api**: En este módulo se modificó el API de `vuelos.py` el cual cuenta con dos nuevos endpoints: `/propiedad-commando` y `/propiedad-query`, los cuales por detrás de escenas usan un patrón CQRS como la base de su comunicación.
    - **modulos/../aplicacion**: Este módulo ahora considera los sub-módulos: `queries` y `comandos`. En dichos directorios pdrá ver como se desacopló las diferentes operaciones lectura y escritura. Vea en el módulo `vuelos` los archivos `obtener_propiedad.py` y `crear_propiedad.py` para ver como se logra dicho desacoplamiento.
...
```bash
docker build . -f adaptador.Dockerfile -t propiedadesalpes/adaptador
```
...
```bash
docker run -p 50051:50051 propiedadesalpes/adaptador
```
...
```bash
docker build . -f notificacion.Dockerfile -t propiedadesalpes/notificacion
```
...
```bash
docker run propiedadesalpes/notificacion
```
...
```bash
docker build . -f ui.Dockerfile -t propiedadesalpes/ui
```
...
```bash
docker run propiedadesalpes/ui
```
